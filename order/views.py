...
from order.forms import BillingInformationForm, ShippingInformationForm, ShippingMethodForm, PaymentInformationForm, CreditCardForm
...
class CheckoutView(View):
    ...
    def get(self, request):
        ...
        billing_info = BillingInformation.objects.filter(user=request.user).first()
        shipping_info = ShippingInformation.objects.filter(user=request.user).first()
        ...
        shipping_method_form = ShippingMethodForm(instance=shipping_info.shipping_method if shipping_info else None)
    ...
    def post(self, request):
        ...
        billing_form = BillingInformationForm(request.POST, instance=billing_info)
        shipping_form = ShippingInformationForm(request.POST, instance=shipping_info)
        shipping_method_form = ShippingMethodForm(request.POST,
                                                  instance=shipping_info.shipping_method if shipping_info else None)
        credit_card_form = CreditCardForm(request.POST, instance=credit_card_info)
